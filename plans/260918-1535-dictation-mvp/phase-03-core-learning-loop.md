...
## Architecture
- **Diff engine** thuần TS, không phụ thuộc React: `web/src/features/dictation/diff/{normalize,tokenize,diff-words,score}.ts`. `diffWords(expected, typed, {strict}) → Token[]` với `status: correct|wrong|extra|missing|pending`. Thuật toán: LCS trên token đã chuẩn hoá (≤ 60 từ → O(n·m) đủ nhanh), rồi ghép cặp các đoạn không khớp thành `wrong` (thay thế) / `extra` / `missing`.
...
- Create (web): `src/features/library/*`, `src/features/player/{use-segment-player.ts,player-bar.tsx,ab-loop.ts}`, `src/features/dictation/{diff/*,workspace.tsx,segment-input.tsx,diff-view.tsx,summary.tsx,shortcuts.ts}`, `src/features/settings/*` (tốc độ, strict mặc định)
...
1. Diff engine + bảng test trước (logic dễ sai nhất); benchmark 60 từ < 5 ms trong Vitest.
...
4. Player hook: segment play, auto-pause rAF, tốc độ (`preservesPitch`), A-B loop + "Lặp câu này", seek → đổi segment hiện tại, lỗi tải audio + retry.
...
## Success Criteria
- [ ] ≥ 30 test diff pass; token đang gõ dở hiện `pending`, không lộ đáp án.
